    ...
    if src_path.exists():
        for module_dir in src_path.iterdir():
            if module_dir.is_dir() and module_dir.name in structure:
                ...
                structure[module_dir.name] = [f.stem for f in py_files if f.stem != '__init__']
    ...
    if tests_path.exists():
        for test_dir in tests_path.iterdir():
            if test_dir.is_dir():
                ...
                structure['tests'].extend([f"{test_dir.name}/{f.stem}" for f in test_files])
...
def generate_diagrams():
    ...
    try:
        # Render DOT to PNG
        subprocess.run([
            'dot', '-Tpng', str(docs_dir / 'detailed_architecture.dot'),
            '-o', str(docs_dir / 'detailed_architecture.png')
