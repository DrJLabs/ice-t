...
def _run_legacy(
    ...
    with envcontext((('PRE_COMMIT_RUNNING_LEGACY', '1'),)):
        ...
        return subprocess.run(cmd, input=stdin, check=False).returncode, stdin
...
def _run_ns(
    ...
    if hook_type in 'commit-msg' or (hook_type == 'prepare-commit-msg' and len(args) == 1):
        return _ns(hook_type, color, commit_msg_filename=args[0])
    if hook_type == 'prepare-commit-msg' and len(args) == 2:
        return _ns(
            hook_type, color, commit_msg_filename=args[0],
            prepare_commit_message_source=args[1],
        )
    if hook_type == 'prepare-commit-msg' and len(args) == 3:
        return _ns(
            hook_type, color, commit_msg_filename=args[0],
            prepare_commit_message_source=args[1], commit_object_name=args[2],
        )
    if hook_type in {'post-commit', 'pre-merge-commit', 'pre-commit'}:
