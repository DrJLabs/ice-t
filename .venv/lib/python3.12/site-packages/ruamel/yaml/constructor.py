...
class SafeConstructor(BaseConstructor):
    ...
    def construct_yaml_int(self, node: Any) -> int:
        ...
        if value_s.startswith('0o'):
            return sign * int(value_s[2:], 8)
        if self.resolver.processing_version == (1, 1) and value_s[0] == '0':
            return sign * int(value_s, 8)
        if self.resolver.processing_version == (1, 1) and ':' in value_s:
...
for tag in ['null', 'bool', 'int', 'float', 'binary', 'timestamp', 'omap', 'pairs', 'set', 'str', 'seq', 'map']:
...
class RoundTripConstructor(SafeConstructor):
    ...
    def construct_scalar(self, node: Any) -> Any:
        ...
        if node.style == '>' and isinstance(node.value, str):
            ...
            return fss
        if bool(self._preserve_quotes) and isinstance(node.value, str):
            if node.style == "'":
...
for tag in ['null', 'int', 'float', 'binary', 'timestamp', 'omap', 'pairs', 'set', 'str', 'seq', 'map']:
