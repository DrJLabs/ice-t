...
def ignore_vuln_if_needed(
    ...
    if severity:
        if float(severity) < ignore_severity_below:
            reason = f'Ignored by severity rule in policy file, {float(severity)} < {ignore_severity_below}'
...
def precompute_remediations(
    ...
    for vuln in vulns:
        ...
        if vuln.package_name in remediations and str(vuln.analyzed_requirement.specifier) in remediations[vuln.package_name]:
...
def compute_sec_ver(
    ...
    Args:
        ...
        db_full (Dict[str, Any]): The full database.
    """
    for pkg_name in remediations:
        pkg: Package = packages.get(pkg_name)
...
def process_fixes_scan(
    ...
    def get_remmediation_from(spec):
        ...
        try:
            ...
            LOG.error('Error getting recommended version for remediation, ignoring', exc_info=True)
