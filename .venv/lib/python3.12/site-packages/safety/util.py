...
def get_proxy_dict(
    ...
    if proxy_protocol and proxy_host and proxy_port:
        ...
        return {"https": f"{proxy_protocol}://{proxy_host}:{proxy_port!s}"}
    return None
    ...
    for announcement in announcements:
        if announcement.get("type", "").lower() == "primary_announcement":
            try:
                ...
                LOG.debug(f"Failed to build primary announcement: {e!s}")
                return None
...
def build_git_data() -> Dict[str, Any]:
    ...
    def git_command(commandline: List[str]) -> str:
        return (
            subprocess.run(
                commandline, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, check=False
            )
            .stdout.decode("utf-8")
            .strip()
        )
...
class MutuallyExclusiveOption(click.Option):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.mutually_exclusive:
            ex_str = ", ".join(
                [
                    f"{item} with values {self.with_values.get(item)}"
    ...
    def handle_parse_result(  # type: ignore
        ...
        if option_used and (not self.with_values or exclusive_value_used):
            ...
            prohibited = "".join(
                [
                    f"\n * --{item} with {self.with_values.get(item)}"
                    if item in self.with_values
                    else f"\n * {item}"
                    for item in self.mutually_exclusive
                ]
