...
class BaseCommand(ABC):
    ...
    def execute(self, ctx: typer.Context) -> ToolResult:
        with self._filelock:
            ...
            process = subprocess.run(
                args, capture_output=self._capture_output, env=self.env(ctx), check=False
            )
    ...
    def _get_installed_packages(self, ctx: typer.Context) -> List[Dict[str, Any]]:
        ...
        result = subprocess.run(args, capture_output=True, env=self.env(ctx), text=True, check=False)
