...
class GenerateSchema:
    ...
    def match_type(self, obj: Any) -> core_schema.CoreSchema:
        ...
        if _typing_extra.is_callable_type(obj):
            return core_schema.callable_schema()
        if _typing_extra.is_literal_type(obj):
            return self._literal_schema(obj)
        if is_typeddict(obj):
            return self._typed_dict_schema(obj, None)
        if _typing_extra.is_namedtuple(obj):
            return self._namedtuple_schema(obj, None)
        if _typing_extra.is_new_type(obj):
            # NewType, can't use isinstance because it fails <3.10
            return self.generate_schema(obj.__supertype__)
        if obj == re.Pattern:
            return self._pattern_schema(obj)
        if obj is collections.abc.Hashable or obj is typing.Hashable:
        ...
        if isinstance(obj, (FunctionType, LambdaType, MethodType, partial)):
            return self._callable_schema(obj)
        if inspect.isclass(obj) and issubclass(obj, Enum):
            return self._enum_schema(obj)
        if is_zoneinfo_type(obj):
    ...
    def _match_generic_type(self, obj: Any, origin: Any) -> CoreSchema:
        ...
        if origin in (re.Pattern, typing.Pattern):
            return self._pattern_schema(obj)
        ...
        with self.model_type_stack.push(namedtuple_cls), self.defs.get_schema_or_ref(namedtuple_cls) as (
            ...
            with self._types_namespace_stack.push(namedtuple_cls):
                ...
                if not annotations:
                    ...
                    annotations = dict.fromkeys(namedtuple_cls._fields, Any)
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if isinstance(type_param, typing.TypeVar):
            ...
            return self._type_schema()
        if _typing_extra.origin_is_union(get_origin(type_param)):
