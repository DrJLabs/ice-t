...
from scipy._lib._array_api import array_namespace, is_numpy, scipy_namespace_for
...
def _chdtrc(xp, spx):
    ...
    gammaincc = getattr(spx.special, 'gammaincc', None) if spx else None
...
def _betaincc(xp, spx):
    betainc = getattr(spx.special, 'betainc', None) if spx else None
