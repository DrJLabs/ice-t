...
def _factorialx_array_exact(n, k=1):
    ...
    if np.isnan(n).any():
        dt = float
    elif k in _FACTORIALK_LIMITS_64BITS:
...
def _factorialx_wrapper(fname, n, k, exact, extend):
    ...
    if fname == "factorial2":
        ...
        if _is_subdtype(type(k), ["f", "c"]) and extend != "complex":
    ...
    if _is_subdtype(n.dtype, types_requiring_complex) and extend != "complex":
        raise ValueError(msg_needs_complex)
