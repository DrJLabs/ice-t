...
def fftconvolve(in1, in2, mode="full", axes=None):
    ...
    if in1.ndim != in2.ndim:
        raise ValueError("in1 and in2 should have the same dimensionality")
    if in1.size == 0 or in2.size == 0:  # empty arrays
        return np.array([])
        ...
        if zi is not None:
            ...
            if zi.shape != expected_shape:
                ...
                for k in range(zi.ndim):
                    if (k == axis and zi.shape[k] == expected_shape[k]) or (k != axis and zi.shape[k] == expected_shape[k]):
...
def envelope(z: np.ndarray, bp_in: tuple[int | None, int | None] = (1, None), *,
    ...
    * The frequency spacing ``1 / (n*T)`` for corner frequencies of the bandpass filter
    ...
    if not ((isinstance(n_out, int) and n_out > 0) or n_out is None):
...
def resample_poly(x, up, down, axis=0, window=('kaiser', 5.0),
    ...
    if isinstance(window, (list | np.ndarray)):
        ...
        if np.issubdtype(x.dtype, np.complexfloating) or np.issubdtype(x.dtype, np.floating):
