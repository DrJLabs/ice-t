...
class CrystalLexer(ExtendedRegexLexer):
    ...
    tokens = {
        'root': [
            (r'#.*?$', Comment.Single),
            # keywords
            (words(['abstract', 'asm', 'begin', 'break', 'case', 'do', 'else', 'elsif', 'end', 'ensure', 'extend', 'if', 'in', 'include', 'next', 'of', 'private', 'protected', 'require', 'rescue', 'return', 'select', 'self', 'super', 'then', 'unless', 'until', 'when', 'while', 'with', 'yield'], suffix=r'\b'), Keyword),
            (words(['previous_def', 'forall', 'out', 'uninitialized', '__DIR__', '__FILE__', '__LINE__', '__END_LINE__'], prefix=r'(?<!\.)', suffix=r'\b'), Keyword.Pseudo),
            ...
            (words(['instance_sizeof', 'offsetof', 'pointerof', 'sizeof', 'typeof'], prefix=r'(?<!\.)', suffix=r'\b'), Keyword.Pseudo),
            # macros
            (r'(?<!\.)(debugger\b|p!|pp!|record\b|spawn\b)', Name.Builtin.Pseudo),
            # builtins
            (words(['abort', 'at_exit', 'caller', 'exit', 'gets', 'loop', 'main', 'p', 'pp', 'print', 'printf', 'puts', 'raise', 'rand', 'read_line', 'sleep', 'spawn', 'sprintf', 'system'], prefix=r'(?<!\.)', suffix=r'\b'), Name.Builtin),
