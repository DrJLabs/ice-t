...
class Expr:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Expr):
            ...
            if self.op in (Op.INTEGER, Op.REAL) and other.op is Op.COMPLEX:
                return as_complex(self) + other
            if self.op is Op.REAL and other.op is Op.INTEGER:
                return self + as_real(other, kind=self.data[1])
            if self.op is Op.INTEGER and other.op is Op.REAL:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Expr):
            ...
            if self.op is Op.COMPLEX and other.op in (Op.INTEGER, Op.REAL):
                return self * as_complex(other)
            if other.op is Op.COMPLEX and self.op in (Op.INTEGER, Op.REAL):
                return as_complex(self) * other
            if self.op is Op.REAL and other.op is Op.INTEGER:
                return self * as_real(other, kind=self.data[1])
            if self.op is Op.INTEGER and other.op is Op.REAL:
    ...
    def traverse(self, visit, *args, **kwargs):
        ...
        if self.op in (Op.INTEGER, Op.REAL, Op.STRING, Op.SYMBOL):
            return self
        if self.op in (Op.COMPLEX, Op.ARRAY, Op.CONCAT, Op.TERNARY):
