...
class LowLevelIRBuilder:
    ...
    def coerce(
        ...
        *,
        ...
        if (src_type.is_unboxed and target_type.is_unboxed) and not is_runtime_subtype(
            ...
            if is_int_rprimitive(src_type) and is_fixed_width_rtype(target_type):
                return self.coerce_int_to_fixed_width(src, target_type, line)
            if is_fixed_width_rtype(src_type) and is_int_rprimitive(target_type):
                return self.coerce_fixed_width_to_int(src, line)
            if is_short_int_rprimitive(src_type) and is_fixed_width_rtype(target_type):
            ...
            if (
                ...
                # Equivalent types
                return src
            if (is_bool_rprimitive(src_type) or is_bit_rprimitive(src_type)) and is_tagged(
                target_type
            ):
                shifted = self.int_op(
            ...
            if isinstance(src, Integer) and is_float_rprimitive(target_type):
                if is_tagged(src_type):
                    return Float(float(src.value // 2))
                return Float(float(src.value))
            if is_tagged(src_type) and is_float_rprimitive(target_type):
    ...
    def gen_method_call(
        ...
        if (
            ...
            if base.type.class_ir.has_attr(name):
