...
class Parser:
    ...
    def _parse_value(self) -> Item:
        ...
        if c == StringType.SLL.value:
            return self._parse_literal_string()
        if c == BoolType.TRUE.value[0]:
            return self._parse_true()
        if c == BoolType.FALSE.value[0]:
            return self._parse_false()
        if c == "[":
            return self._parse_array()
        if c == "{":
            return self._parse_inline_table()
        if c in "+-" or self._peek(4) in {
            "+inf",
            "-inf",
            "inf",
            "+nan",
    ...
    def _parse_string(self, delim: StringType) -> String:
        ...
        while True:
            ...
            if not escaped and self._current == delim.unit:
