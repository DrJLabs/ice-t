    ...
    def visit_BinOp(self, node: BinOp) -> Any:
        ...
        if isinstance(node.op, BitOr):
            ...
            if self._memo.name_matches(node.left, *anytype_names):
                return node.left
            if self._memo.name_matches(node.right, *anytype_names):
                return node.right
        ...
        if node.slice:
            ...
            if isinstance(slice_value, Tuple):
                ...
                if self._memo.name_matches(
                    ...
                    return None
                if (sys.version_info >= (3, 9) and not hasattr(node, "slice")) or (sys.version_info < (3, 9) and not hasattr(node.slice, "value")):
                    return node.value
            ...
            if self._memo.memo_var_name:
                ...
                while memo:
                    if isinstance(memo.node, (FunctionDef, AsyncFunctionDef)):
                        ...
                        del names[:-1]
                        break
                    if not isinstance(memo.node, ClassDef):
                        break
