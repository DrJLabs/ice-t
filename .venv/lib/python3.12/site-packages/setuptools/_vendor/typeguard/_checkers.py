...
def check_callable(
    ...
    if args:
        ...
        if isinstance(argument_types, list) and not any(
            ...
            if not has_varargs and num_positional_args < len(argument_types):
                raise TypeCheckError(
...
def check_number(
    ...
    if origin_type is float and not isinstance(value, (float, int)):
...
def builtin_checker_lookup(
    ...
    if isclass(origin_type) and issubclass(
        origin_type,
        Tuple,  # type: ignore[arg-type]
    ):
        # NamedTuple
        return check_tuple
    if getattr(origin_type, "_is_protocol", False):
        return check_protocol
    if isinstance(origin_type, ParamSpec):
        return check_paramspec
    if isinstance(origin_type, TypeVar):
        return check_typevar
    if origin_type.__class__ is NewType:
