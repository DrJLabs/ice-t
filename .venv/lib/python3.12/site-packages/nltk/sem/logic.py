...
class LogicParser:
    ...
    def handle(self, tok, context):
        ...
        if tok in Tokens.NOT_LIST:
            return self.handle_negation(tok, context)

        if tok in Tokens.LAMBDA_LIST:
            return self.handle_lambda(tok, context)

        if tok in Tokens.QUANTS:
    ...
    def get_QuantifiedExpression_factory(self, tok):
        ...
        if tok in Tokens.ALL_LIST:
            return AllExpression
        if tok in Tokens.IOTA_LIST:
    ...
    def get_BooleanExpression_factory(self, tok):
        ...
        if tok in Tokens.OR_LIST:
            return OrExpression
        if tok in Tokens.IMP_LIST:
            return ImpExpression
        if tok in Tokens.IFF_LIST:
...
def read_type(type_string):
    ...
    if type_string[0] == "%s" % ENTITY_TYPE:
        return ENTITY_TYPE
    if type_string[0] == "%s" % TRUTH_TYPE:
        return TRUTH_TYPE
    if type_string[0] == "%s" % ANY_TYPE:
        return ANY_TYPE
    raise LogicalExpressionException(
    ...
    def normalize(self, newvars=None):
        ...
        def get_indiv_vars(e):
            if isinstance(e, IndividualVariableExpression):
                return {e}
            if isinstance(e, AbstractVariableExpression):
                return set()
            return e.visit(
...
def VariableExpression(variable):
    ...
    if is_funcvar(variable.name):
        return FunctionVariableExpression(variable)
    if is_eventvar(variable.name):
