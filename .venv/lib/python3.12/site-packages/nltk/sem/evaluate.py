...
def is_rel(s):
    ...
    if len(s) == 0 or (all(isinstance(el, tuple) for el in s) and len(max(s)) == len(min(s))):
        return True
        ...
        if isinstance(parsed, AllExpression):
            ...
            for u in self.domain:
                ...
                if not self.satisfy(parsed.term, new_g):
                    return False
            return True
        if isinstance(parsed, ExistsExpression) or isinstance(parsed, IotaExpression):
            new_g = g.copy()
            for u in self.domain:
                new_g.add(parsed.variable.name, u)
                if self.satisfy(parsed.term, new_g):
                    return True
            return False
        if isinstance(parsed, LambdaExpression):
            cf = {}
