...
def _pretty_semtype(st):
    ...
    outstr = ""
    outstr += f"semantic type ({st.ID}): {st.name}\n"
...
def _pretty_frame_relation_type(freltyp):
    ...
    :rtype: str
    """
    outstr = f"<frame relation type ({freltyp.ID}): {freltyp.superFrameName} -- {freltyp.name} -> {freltyp.subFrameName}>"
    return outstr
...
def _pretty_frame_relation(frel):
    ...
    :rtype: str
    """
    outstr = f"<{frel.type.superFrameName}={frel.superFrameName} -- {frel.type.name} -> {frel.type.subFrameName}={frel.subFrameName}>"
    return outstr
...
def _pretty_fe_relation(ferel):
    ...
    :rtype: str
    """
    outstr = f"<{ferel.type.superFrameName}={ferel.frameRelation.superFrameName}.{ferel.superFEName} -- {ferel.type.name} -> {ferel.type.subFrameName}={ferel.frameRelation.subFrameName}.{ferel.subFEName}>"
    return outstr
...
def _pretty_lu(lu):
    ...
    lukeys = lu.keys()
    outstr = ""
    outstr += f"lexical unit ({lu.ID}): {lu.name}\n\n"
    ...
    if "exemplars" in lukeys:
        outstr += f"\n[exemplars] {len(lu.exemplars)} sentences across all subcorpora\n"
...
def _pretty_exemplars(exemplars, lu):
    ...
    outstr = ""
    outstr += f"exemplar sentences for {lu.name} in {lu.frame.name}:\n\n"
...
def _pretty_fulltext_sentences(sents):
    ...
    outstr = ""
    outstr += f"full-text document ({sents.ID}) {sents.name}:\n\n"
    outstr += f"[corpid] {sents.corpid}\n[corpname] {sents.corpname}\n[description] {sents.description}\n[URL] {sents.URL}\n\n"
    outstr += "[sentence]\n"
...
def _pretty_pos(aset):
    ...
    outstr = ""
    outstr += f"POS annotation set ({aset.ID}) {aset.POS_tagset} in sentence {aset.sent.ID}:\n\n"
...
def _pretty_annotation(sent, aset_level=False):
    ...
    for k in ("corpID", "docID", "paragNo", "sentNo", "aPos"):
        if k in sentkeys:
            outstr += f"[{k}] {sent[k]}\n"
    outstr += (
        f"\n[LU] ({sent.LU.ID}) {sent.LU.name} in {sent.LU.frame.name}\n"
        if sent.LU
        else "\n[LU] Not found!"
    )
    outstr += f"\n[frame] ({sent.frame.ID}) {sent.frame.name}\n"  # redundant with above, but .frame is convenient
    if not aset_level:
        outstr += f"\n[annotationSet] {len(sent.annotationSet)} annotation sets\n"
        outstr += f"\n[POS] {len(sent.POS)} tags\n"
        outstr += f"\n[POS_tagset] {sent.POS_tagset}\n"
    outstr += "\n[GF] {} relation{}\n".format(
...
def _annotation_ascii(sent):
    ...
    # an FE annotation set, or an LU sentence with 1 target
...
def _pretty_fe(fe):
    ...
    fekeys = fe.keys()
    outstr = ""
    outstr += f"frame element ({fe.ID}): {fe.name}\n    of {fe.frame.name}({fe.frame.ID})\n"
...
def _pretty_frame(frame):
    ...
    outstr = ""
    outstr += f"frame ({frame.ID}): {frame.name}\n\n"
    ...
    outstr += f"\n[frameRelations] {len(frame.frameRelations)} frame relations\n"
    ...
    outstr += f"\n[FEcoreSets] {len(frame.FEcoreSets)} frame element core sets\n"
    outstr += (
        "  "
        + "\n  ".join(
...
warnings(True) to display corpus consistency warnings when loading data
    ...
    def lu(self, fn_luid, ignorekeys=[], luName=None, frameID=None, frameName=None):
        ...
        if luinfo is OOV:
            ...
            self._warn(
                f"LU ID not found: {luName} ({fn_luid}) in {frameName} ({frameID})"
            )
            luinfo = AttrDict(
                {
                    "_type": "lu",
    ...
    def exemplars(self, luNamePattern=None, frame=None, fe=None, fe2=None):
        ...
        if fe is not None and fe2 is not None:
    ...
    def frame_relations(self, frame=None, frame2=None, type=None):
        ...
        if frame2 is not None:
            ...
            rels = [
                rel
                for rel in rels
                if frame2 == rel.superFrame.ID or frame2 == rel.subFrame.ID
            ]
    ...
    def _handle_lexunit_elt(self, elt, ignorekeys):
        ...
        for sub in elt:
            if sub.tag.endswith("header") or sub.tag.endswith("valences"):
                continue  # not used
            if sub.tag.endswith("definition") and "definition" not in ignorekeys:
                luinfo["definitionMarkup"] = sub.text
                luinfo["definition"] = self._strip_tags(sub.text)
            ...
            if sub.tag.endswith("layer"):
                ...
                if l is not None:
                    ...
                    if l.name == "Target":
                        if not overt:
                            self._warn(
                                f"Skipping empty Target layer in annotation set ID={info.ID}"
                            )
                            continue
                        assert all(lblname == "Target" for i, j, lblname in overt)
                        if "Target" in info:
                            self._warn(
                                f"Annotation set {info.ID} has multiple Target layers"
                            )
                        else:
...
def demo():
    ...
    #
    print(
        f'\nNumber of frame relations for the "{m_frame.name}" ({m_frame.ID}) frame:',
