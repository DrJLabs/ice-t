...
class DecisionTreeClassifier(ClassifierI):
    ...
    def pseudocode(self, prefix="", depth=4):
        ...
        if self._default is not None:
            if len(self._decisions) == 1:
                s += f"{prefix}if {self._fname} != {list(self._decisions.keys())[0]!r}: "
            else:
                s += f"{prefix}else: "
            if self._default._fname is not None and depth > 1:
    ...
    def best_stump(feature_names, labeled_featuresets, verbose=False):
        ...
        if verbose:
            print(
                f"best stump for {len(labeled_featuresets):6d} toks uses {best_stump._fname:20} err={best_error:6.4f}"
            )
        return best_stump
    ...
    def best_binary_stump(
        ...
        if verbose:
            if best_stump._decisions:
                ...
                descr = "(default)"
            print(
                f"best stump for {len(labeled_featuresets):6d} toks uses {descr:20} err={best_error:6.4f}"
            )
        return best_stump
