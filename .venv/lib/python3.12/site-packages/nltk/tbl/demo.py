...
def demo_generated_templates():
    ...
    print(
        f"Generated {len(templates)} templates for transformation-based learning"
    )
...
def postag(
    ...
    if cache_baseline_tagger:
        if not os.path.exists(cache_baseline_tagger):
            ...
            print(
                f"Trained baseline tagger, pickled it to {cache_baseline_tagger}"
            )
    ...
    if gold_data:
        print(
            f"    Accuracy on test set: {baseline_tagger.accuracy(gold_data):0.4f}"
        )
