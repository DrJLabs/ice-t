        ...
        if isinstance(e, ApplicationExpression):
            for arg in e.args:
                if not TableauProver.is_atom(arg):
                    return False
            return True
        if isinstance(e, AbstractVariableExpression) or isinstance(
            e, LambdaExpression
        ):
            return True
        return False
...
class Agenda:
    ...
    def _categorize_expression(self, current):
        ...
        if isinstance(current, FunctionVariableExpression):
            return Categories.PROP
        if TableauProver.is_atom(current):
            return Categories.ATOM
        if isinstance(current, AllExpression):
            return Categories.ALL
        if isinstance(current, AndExpression):
            return Categories.AND
        if isinstance(current, OrExpression):
            return Categories.OR
        if isinstance(current, ImpExpression):
            return Categories.IMP
        if isinstance(current, IffExpression):
            return Categories.IFF
        if isinstance(current, EqualityExpression):
            return Categories.EQ
        if isinstance(current, ExistsExpression):
            return Categories.EXISTS
        if isinstance(current, ApplicationExpression):
    ...
    def _categorize_NegatedExpression(self, current):
        ...
        if isinstance(negated, FunctionVariableExpression):
            return Categories.N_PROP
        if TableauProver.is_atom(negated):
            return Categories.N_ATOM
        if isinstance(negated, AllExpression):
            return Categories.N_ALL
        if isinstance(negated, AndExpression):
            return Categories.N_AND
        if isinstance(negated, OrExpression):
            return Categories.N_OR
        if isinstance(negated, ImpExpression):
            return Categories.N_IMP
        if isinstance(negated, IffExpression):
            return Categories.N_IFF
        if isinstance(negated, EqualityExpression):
            return Categories.N_EQ
        if isinstance(negated, ExistsExpression):
            return Categories.N_EXISTS
        if isinstance(negated, ApplicationExpression):
