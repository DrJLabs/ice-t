...
def _complete_unify_path(first, second, bindings, used, skipped, debug):
    ...
    # no bindings made means no unification occurred.  so no result
    debug.line("  -> End")
    return []
...
def _clausify(expression):
    ...
    if isinstance(expression, EqualityExpression) or isinstance(expression, ApplicationExpression):
        return [Clause([expression])]
    if isinstance(expression, NegatedExpression):
        if isinstance(expression.term, ApplicationExpression) or isinstance(expression.term, EqualityExpression):
            return [Clause([expression])]
...
def most_general_unification(a, b, bindings=None):
    ...
    if a == b:
        return bindings
    if isinstance(a, IndividualVariableExpression):
        return _mgu_var(a, b, bindings)
    if isinstance(b, IndividualVariableExpression):
        return _mgu_var(b, a, bindings)
    if isinstance(a, ApplicationExpression) and isinstance(b, ApplicationExpression):
