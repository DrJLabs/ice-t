    ...
    def status(self, info_or_id, download_dir=None):
        ...
        if isinstance(info, Collection):
            ...
            if self.STALE in pkg_status:
                return self.STALE
            if self.PARTIAL in pkg_status or (self.INSTALLED in pkg_status and self.NOT_INSTALLED in pkg_status):
                return self.PARTIAL
            if self.NOT_INSTALLED in pkg_status:
        ...
        for collection in self._collections.values():
            for i, child_id in enumerate(collection.children):
                if child_id in self._packages:
                    ...
                    print(
                        f"removing collection member with no package: {child_id}"
                    )
...
class DownloaderShell:
    ...
    def _simple_interactive_update(self):
        while True:
            stale_packages = []
            stale = partial = False
            for info in sorted(self._ds.packages(), key=str):
