    ...
    def __init__(self, ignore_stopwords=False):
        ...
        if ignore_stopwords:
            try:
                ...
                raise ValueError(
                    f"{self!r} has no list of stopwords. Please set"
                    " 'ignore_stopwords' to 'False'."
                ) from e
        ...
        for suffix in self.__step2_suffixes:
            if word.endswith(suffix):
                if r1.endswith(suffix):
                    ...
                    elif suffix in ("fulli", "lessli") or (suffix == "li" and word[-3] in self.__li_ending):
                        word = word[:-2]
                        r1 = r1[:-2]
                        r2 = r2[:-2]
                break
            ...
            if r1.endswith(suffix):
                if suffix == "sti":
                    if suffix in r2:
                        ...
                        r1 = r1[:-3]
                        r2 = r2[:-3]
                elif word[-len(suffix) - 1] in "ntaeiouy\xE4\xF6":
                    word = word[: -len(suffix)]
                    r1 = r1[: -len(suffix)]
                    r2 = r2[: -len(suffix)]
                break
