...
class BrillTagger(TaggerI):
    ...
    def print_template_statistics(self, test_stats=None, printunused=True):
        ...
        assert len(trainscores) == len(
            tids
        ), "corrupt statistics: " f"{trainscores} train scores for {tids} rules"
        ...
        def print_train_stats():
            print(
                f"TEMPLATE STATISTICS (TRAIN)  {len(template_counts)} templates, {len(tids)} rules)"
            )
            print(
            ...
            for tid, trainscore in train_tplscores:
                s = f"{tid} | {trainscore:5d}   {trainscore / tottrainscores:5.3f} |{template_counts[tid]:4d}   {template_counts[tid] / len(tids):.3f} | {Template.ALLTEMPLATES[int(tid)]}"
                print(s)

        def print_testtrain_stats():
            testscores = test_stats["rulescores"]
            print(
                f"TEMPLATE STATISTICS (TEST AND TRAIN) ({len(template_counts)} templates, {len(tids)} rules)"
            )
            print(
            ...
            for tid, testscore in test_tplscores:
                s = f"{tid:s} |{testscore:5d}  {testscore / tottestscores:6.3f} |  {weighted_traincounts[tid]:4d}   {weighted_traincounts[tid] / tottrainscores:.3f} |{template_counts[tid]:4d}   {template_counts[tid] / len(tids):.3f} | {Template.ALLTEMPLATES[int(tid)]:s}"
                print(s)
