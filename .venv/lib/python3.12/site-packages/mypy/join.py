...
class TypeJoinVisitor(TypeVisitor[ProperType]):
    ...
    def visit_none_type(self, t: NoneType) -> ProperType:
        if state.strict_optional:
            if isinstance(self.s, (NoneType, UninhabitedType)):
                return t
            if isinstance(self.s, (UnboundType, AnyType)):
                return AnyType(TypeOfAny.special_form)
            return mypy.typeops.make_simplified_union([self.s, t])
        return self.s
    ...
    def visit_instance(self, t: Instance) -> ProperType:
        ...
        if isinstance(self.s, TypeType) or isinstance(self.s, TypedDictType) or isinstance(self.s, TupleType) or isinstance(self.s, LiteralType):
            return join_types(t, self.s)
        if isinstance(self.s, TypeVarTupleType) and is_subtype(self.s.upper_bound, t):
            return t
        return self.default(self.s)
    ...
    def visit_callable_type(self, t: CallableType) -> ProperType:
        if isinstance(self.s, CallableType) and is_similar_callables(t, self.s):
            ...
            return result
        if isinstance(self.s, Overloaded):
    ...
    def visit_overloaded(self, t: Overloaded) -> ProperType:
        ...
        if isinstance(s, Instance) and s.type.is_protocol:
    ...
    def default(self, typ: Type) -> ProperType:
        ...
        if isinstance(typ, TypedDictType) or isinstance(typ, FunctionLike):
            return self.default(typ.fallback)
        if isinstance(typ, TypeVarType) or isinstance(typ, ParamSpecType):
...
def object_or_any_from_type(typ: ProperType) -> ProperType:
    ...
    if isinstance(typ, Instance):
        return object_from_instance(typ)
    if isinstance(typ, (CallableType, TypedDictType, LiteralType)):
    ...
    if isinstance(typ, TypeVarLikeType) and isinstance(typ.upper_bound, ProperType):
