...
def read_quickstart_file(
    ...
    if options.quickstart_file:
        ...
        try:
            ...
            print(f"Warning: Failed to load quickstart file: {e!s}\n", file=stdout)
    return quickstart
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> CacheMeta | None:
    ...
    if not isinstance(meta, dict):
        manager.log(f"Could not load cache for {id}: meta cache is not a dict: {meta!r}")  # type: ignore[unreachable]
        return None
            ...
            if self.path and source is None:
                try:
                    ...
                    if self.path.endswith(".pyd"):
                        ...
                        err = f"mypy: can't decode file '{self.path}': {decodeerr!s}"
                ...
                for id in all_ids:
                    if id in scc:
                        if graph[id].xmeta.data_mtime < newest_in_deps:
                            ...
                            key = "id:"
                    elif graph[id].xmeta.data_mtime > oldest_in_scc:
                        key = "+dep:"
                    else:
                        key = "dep:"
