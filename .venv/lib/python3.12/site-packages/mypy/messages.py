...
class MessageBuilder:
    ...
    def has_no_attr(
        ...
        if member in op_methods.values():
            ...
            if isinstance(original_type, TypeVarType):
        ...
        if conflict_types and (
            ...
            for name, got, exp, is_lvalue in conflict_types[:MAX_ITEMS]:
                ...
                if not isinstance(exp, (CallableType, Overloaded)) or not isinstance(
                    ...
                    self.note(
                        f"Expected{setter_suffix}:", context, offset=OFFSET, code=code
                    )
    ...
    def try_report_long_tuple_assignment_error(
        ...
        if isinstance(subtype, TupleType):
            if (
                ...
                return True
            if isinstance(supertype, TupleType) and (
    ...
    def generate_incompatible_tuple_error(
        ...
        for i, (lhs_t, rhs_t) in enumerate(zip(lhs_types, rhs_types)):
            if not is_subtype(rhs_t, lhs_t):
                if error_cnt < 3:
                    notes.append(
                        f"Expression tuple item {i!s} has type {format_type(rhs_t, self.options)}; {format_type(lhs_t, self.options)} expected; "
                    )
                error_cnt += 1

        info = f" ({error_cnt!s} tuple items are incompatible"
        if error_cnt - 3 > 0:
            info += f"; {error_cnt - 3!s} items are omitted)"
        else:
            info += ")"
...
def format_type_inner(
    ...
    if isinstance(typ, Instance):
        ...
        if not itype.args:
            ...
            return base_str
        if itype.type.fullname == "builtins.tuple":
    ...
    if isinstance(typ, TypeVarType) or isinstance(typ, TypeVarTupleType):
