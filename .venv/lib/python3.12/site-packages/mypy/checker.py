...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def is_unsafe_overlapping_op(
        ...
        if is_same_type(reverse_base_erased, forward_base_erased):
            return False
        if is_subtype(reverse_base_erased, forward_base_erased):
    ...
    def check_enum_bases(self, defn: ClassDef) -> None:
        ...
        for base in defn.info.bases:
            if enum_base is None and base.type.is_enum:
                enum_base = base
                continue
            if enum_base is not None and not base.type.is_enum:
                self.fail(
    ...
    def check_compatibility_classvar_super(
        ...
        if node.is_classvar and not base_node.is_classvar:
            ...
            return False
        if not node.is_classvar and base_node.is_classvar:
            ...
            for idx_rval, rval in enumerate(self.flatten_rvalues(rvalue.items)):
                if isinstance(rval, StarExpr):
                    ...
                    if self.type_is_iterable(typs) and isinstance(typs, Instance):
                        ...
                        elif last_idx is None or last_idx + 1 == idx_rval:
                            rvalues.append(rval)
                            last_idx = idx_rval
    ...
    def refers_to_different_scope(self, name: NameExpr) -> bool:
        if name.kind == LDEF:
            ...
            return False
        if self.scope.top_level_function() is not None:
            # A non-local reference from within a function must refer to a different scope
            return True
        if name.kind == GDEF and name.fullname.rpartition(".")[0] != self.tree.fullname:
    ...
    def lookup_qualified(self, name: str) -> SymbolTableNode:
        ...
        if last in n.names:
            return n.names[last]
        if len(parts) == 2 and parts[0] in ("builtins", "typing"):
            fullname = ".".join(parts)
    ...
    def is_writable_attribute(self, node: Node) -> bool:
        ...
        if isinstance(node, Var):
            if node.is_property and not node.is_settable_property:
                return False
            return True
        if isinstance(node, OverloadedFuncDef) and node.is_property:
...
def flatten_types(t: Type) -> list[Type]:
    ...
    if is_named_instance(t, "builtins.tuple"):
        return [t.args[0]]
    return [t]
