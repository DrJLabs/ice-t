...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def visit_unbound_type_nonoptional(self, t: UnboundType, defining_literal: bool) -> Type:
        ...
        if sym is not None:
            ...
            if node.fullname in TYPE_ALIAS_NAMES:
                return AnyType(TypeOfAny.special_form)
    ...
    def apply_concatenate_operator(self, t: UnboundType) -> Type:
        ...
        if not isinstance(ps, (ParamSpecType, Parameters)):
            ...
            return AnyType(TypeOfAny.from_error)
        if isinstance(ps, ParamSpecType) and ps.prefix.arg_types:
    ...
    def try_analyze_special_unbound_type(self, t: UnboundType, fullname: str) -> Type | None:
        ...
        if fullname in FINAL_TYPE_NAMES:
        ...
        if fullname in NEVER_NAMES:
            return UninhabitedType()
        if fullname in LITERAL_TYPE_NAMES:
            return self.analyze_literal_type(t)
        if fullname in ANNOTATED_TYPE_NAMES:
            if len(t.args) < 2:
                self.fail(
        ...
        if fullname in ("typing_extensions.Required", "typing.Required"):
        ...
        if fullname in ("typing_extensions.NotRequired", "typing.NotRequired"):
            if not self.allow_typed_dict_special_forms:
                self.fail(
        ...
        if fullname in ("typing_extensions.ReadOnly", "typing.ReadOnly"):
            if not self.allow_typed_dict_special_forms:
                self.fail(
        ...
        if fullname in UNPACK_TYPE_NAMES:
            if len(t.args) != 1:
            ...
            self.allow_type_var_tuple = False
            return result
        if fullname in SELF_TYPE_NAMES:
            if t.args:
        ...
        if len(t.args) == 0:
            ...
            if isinstance(callable_args, TypeList):
                ...
                if maybe_ret is None:
                    ...
                    return AnyType(TypeOfAny.from_error)
                if isinstance(maybe_ret, AnyType):
                    return maybe_ret
                ret = maybe_ret
        else:
        ...
        for i, arg in enumerate(arglist.items):
            if isinstance(arg, CallableArgument):
                ...
                if found is None:
                    ...
                    return None
                if found.fullname not in ARG_KINDS_BY_CONSTRUCTOR:
    ...
    def analyze_literal_param(self, idx: int, arg: Type, ctx: Context) -> list[Type] | None:
        ...
        if isinstance(arg, (NoneType, LiteralType)):
            # Types that we can just add directly to the literal/potential union of literals.
            return [arg]
        if isinstance(arg, Instance) and arg.last_known_value is not None:
