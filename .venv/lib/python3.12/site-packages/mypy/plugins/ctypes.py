    ...
    if et is not None:
        ...
        for arg_num, (arg_kind, arg_type) in enumerate(zip(ctx.arg_kinds[0], ctx.arg_types[0]), 1):
            if arg_kind == nodes.ARG_POS and not is_subtype(arg_type, allowed):
                ctx.api.msg.fail(
                    f"Array constructor argument {arg_num} of type {format_type(arg_type, ctx.api.options)}"
                    f" is not convertible to the array element type {format_type(et, ctx.api.options)}",
                    ctx.context,
                )
                ...
                if not is_subtype(arg_type, ty):
                    ...
                    ctx.api.msg.fail(
                        f"Array constructor argument {arg_num} of type {format_type(arg_type, ctx.api.options)}"
                        f" is not convertible to the array element type {format_type(it, ctx.api.options)}",
                        ctx.context,
                    )
    ...
    if et is not None:
        ...
        if isinstance(index_type, Instance):
            if index_type.type.has_base("builtins.int"):
                return unboxed
            if index_type.type.has_base("builtins.slice"):
        ...
        for tp in flatten_nested_unions([et]):
            ...
            if isinstance(tp, AnyType):
                ...
                ctx.api.msg.fail(
                    ...
                    f' with element type "c_char" or "c_wchar", not {format_type(et, ctx.api.options)}',
                    ctx.context,
                )
        ...
        for tp in flatten_nested_unions([et]):
            ...
            if (
                ...
                ctx.api.msg.fail(
                    ...
                    f' with element type "c_char", not {format_type(et, ctx.api.options)}',
                    ctx.context,
                )
        return make_simplified_union(types)
    return ctx.default_attr_type
