...
class DefaultPlugin(Plugin):
    ...
    def get_function_signature_hook(
        ...
        if fullname in ("attr.fields", "attrs.fields"):
    ...
    def get_method_signature_hook(
        ...
        if fullname in {n + ".setdefault" for n in TPDICT_FB_NAMES}:
            return typed_dict_setdefault_signature_callback
        if fullname in {n + ".pop" for n in TPDICT_FB_NAMES}:
            return typed_dict_pop_signature_callback
        if fullname == "_ctypes.Array.__setitem__":
            return ctypes.array_setitem_callback
        if fullname == singledispatch.SINGLEDISPATCH_CALLABLE_CALL_METHOD:
    ...
    def get_method_hook(self, fullname: str) -> Callable[[MethodContext], Type] | None:
        ...
        if fullname == "builtins.int.__neg__":
            return int_neg_callback
        if fullname == "builtins.int.__pos__":
            return int_pos_callback
        if fullname in ("builtins.tuple.__mul__", "builtins.tuple.__rmul__"):
            return tuple_mul_callback
        if fullname in {n + ".setdefault" for n in TPDICT_FB_NAMES}:
            return typed_dict_setdefault_callback
        if fullname in {n + ".pop" for n in TPDICT_FB_NAMES}:
            return typed_dict_pop_callback
        if fullname in {n + ".__delitem__" for n in TPDICT_FB_NAMES}:
            return typed_dict_delitem_callback
        if fullname == "_ctypes.Array.__getitem__":
            return ctypes.array_getitem_callback
        if fullname == "_ctypes.Array.__iter__":
            return ctypes.array_iter_callback
        if fullname == singledispatch.SINGLEDISPATCH_REGISTER_METHOD:
            return singledispatch.singledispatch_register_callback
        if fullname == singledispatch.REGISTER_CALLABLE_CALL_METHOD:
            return singledispatch.call_singledispatch_function_after_register_argument
        if fullname == "functools.partial.__call__":
            import mypy.plugins.functools
    ...
    def get_attribute_hook(self, fullname: str) -> Callable[[AttributeContext], Type] | None:
        ...
        if fullname == "_ctypes.Array.raw":
            return ctypes.array_raw_callback
        if fullname in enums.ENUM_NAME_ACCESS:
            return enums.enum_name_callback
        if fullname in enums.ENUM_VALUE_ACCESS:
