...
def narrow_declared_type(declared: Type, narrowed: Type) -> Type:
    ...
    if isinstance(narrowed, AnyType):
        return original_narrowed
    if isinstance(narrowed, TypeVarType) and is_subtype(narrowed.upper_bound, declared):
        return narrowed
    if isinstance(declared, TypeType) and isinstance(narrowed, TypeType):
    ...
    if isinstance(declared, (TupleType, TypeType, LiteralType)):
        return meet_types(original_declared, original_narrowed)
    if isinstance(declared, TypedDictType) and isinstance(narrowed, Instance):
...
def is_overlapping_types(
    ...
    if typed_dict_mapping_pair(left, right):
    ...
    def _type_object_overlap(left: Type, right: Type) -> bool:
        ...
        if isinstance(left, TypeType) and isinstance(right, Instance):
            ...
            if isinstance(left.item, AnyType):
                return right.type.has_base("builtins.type")
    ...
    def visit_unbound_type(self, t: UnboundType) -> ProperType:
        if isinstance(self.s, NoneType):
            if state.strict_optional:
                return UninhabitedType()
            return self.s
        if isinstance(self.s, UninhabitedType):
            return self.s
    ...
    def visit_instance(self, t: Instance) -> ProperType:
        if isinstance(self.s, Instance):
            ...
            if state.strict_optional:
                return UninhabitedType()
            return NoneType()
        if isinstance(self.s, FunctionLike) and t.type.is_protocol:
    ...
    def visit_callable_type(self, t: CallableType) -> ProperType:
        if isinstance(self.s, CallableType) and join.is_similar_callables(t, self.s):
            ...
            if isinstance(get_proper_type(result.ret_type), UninhabitedType):
                ...
                return self.default(self.s)
            return result
        if isinstance(self.s, TypeType) and t.is_type_obj() and not t.is_generic():
            ...
            return self.default(self.s)
        if isinstance(self.s, Instance) and self.s.type.is_protocol:
    ...
    def visit_tuple_type(self, t: TupleType) -> ProperType:
        ...
        if isinstance(self.s, Instance):
            ...
            if is_proper_subtype(t, self.s):
                ...
                return t
            if self.s.type.has_type_var_tuple_type and is_subtype(t, self.s):
