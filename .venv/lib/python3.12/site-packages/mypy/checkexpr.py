...
class ExpressionChecker(ExpressionVisitor[Type], ExpressionCheckerSharedApi):
    ...
    def dangerous_comparison(
        ...
        *,
        ...
        if isinstance(left, Instance) and isinstance(right, Instance):
            ...
            if left.type.has_base("typing.Mapping") and right.type.has_base("typing.Mapping"):
            ...
            if left_name in ("builtins.list", "builtins.tuple") and right_name == left_name:
                return self.dangerous_comparison(
                    left.args[0], right.args[0], seen_types=seen_types
                )
            if left_name in OVERLAPPING_BYTES_ALLOWLIST and right_name in (
    ...
    def visit_index_with_type(
        ...
        if isinstance(left_type, TypedDictType):
            return self.visit_typeddict_index_expr(left_type, e.index)[0]
        if isinstance(left_type, FunctionLike) and left_type.is_type_obj():
            if left_type.type_object().is_enum:
                return self.visit_enum_index_expr(left_type.type_object(), e.index, e)
            if self.chk.options.python_version >= (3, 9) and (
        ...
        if isinstance(left_type, Instance) and left_type.type.fullname == "typing._SpecialForm":
    ...
    def _super_arg_types(self, e: SuperExpr) -> Type | tuple[Type, Type]:
        ...
        if len(e.call.args) == 0:
            if not e.info:
                ...
                return AnyType(TypeOfAny.from_error)
            if self.chk.scope.active_class():
