...
class TestSystemSwapMemory(PsutilTestCase):
    ...
    def test_no_vmstat_mocked(self):
        ...
        with mock_open_exception(
            "/proc/vmstat", OSError(errno.ENOENT, 'no such file or directory')
        ) as m:
            with warnings.catch_warnings(record=True) as ws:
                warnings.simplefilter("always")
                ret = psutil.swap_memory()
...
class TestSystemCPUFrequency(PsutilTestCase):
    ...
    def test_emulate_data(self):
        def open_mock(name, *args, **kwargs):
            ...
            if name.endswith('/scaling_min_freq') and name.startswith(
                "/sys/devices/system/cpu/cpufreq/policy"
            ):
                return io.BytesIO(b"600000")
            if name.endswith('/scaling_max_freq') and name.startswith(
                "/sys/devices/system/cpu/cpufreq/policy"
            ):
    ...
    def test_emulate_multi_cpu(self):
        def open_mock(name, *args, **kwargs):
            ...
            if n.endswith('/scaling_min_freq') and n.startswith(
                "/sys/devices/system/cpu/cpufreq/policy0"
            ):
                return io.BytesIO(b"200000")
            if n.endswith('/scaling_max_freq') and n.startswith(
                "/sys/devices/system/cpu/cpufreq/policy0"
            ):
...
class TestSensorsBattery(PsutilTestCase):
    ...
    def test_emulate_no_power(self):
        ...
        with mock_open_exception(
            "/sys/class/power_supply/AC/online", OSError(errno.ENOENT, "")
        ):
            with mock_open_exception(
                "/sys/class/power_supply/AC0/online", OSError(errno.ENOENT, "")
            ):
                with mock_open_exception(
...
class TestSensorsBatteryEmulated(PsutilTestCase):
    def test_it(self):
        def open_mock(name, *args, **kwargs):
            ...
            if name.endswith("/power_now"):
                return io.StringIO("0")
            if name.endswith("/energy_full"):
...
class TestProcess(PsutilTestCase):
    ...
    def test_open_files_fd_gone(self):
        ...
        with open(self.get_testfn(), 'w'):
            ...
            patch_point = 'builtins.open' if PY3 else '__builtin__.open'
            with mock.patch(
                patch_point, side_effect=OSError(errno.ENOENT, "")
            ) as m:
                assert p.open_files() == []
                assert m.called
    ...
    def test_issue_1014(self):
        ...
        with mock_open_exception(
            '/proc/%s/smaps' % os.getpid(), OSError(errno.ENOENT, "")
        ) as m:
            p = psutil.Process()
            with pytest.raises(FileNotFoundError):
                p.memory_maps()
